...
4. Feature-input layer: implemented under `hose_quant.data`. It prepares auditable current-snapshot universe candidates, backward-looking liquidity characterizations, long-form daily panels, data-availability diagnostics, versioned contracts, market-time policy, and evidence-derived unit permission. Generated outputs live under `data/feature_inputs/vnstock/`.
