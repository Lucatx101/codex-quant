...
3. Normalized provider market data: implemented for Phase 1 Parquet datasets under `data/normalized/vnstock/`. These datasets do not imply historical universe membership.
4. Feature-input layer: implemented under `hose_quant.data`. It prepares auditable current-snapshot universe candidates, backward-looking liquidity characterizations, long-form daily panels, data-availability diagnostics, versioned contracts, and market-time policy. Generated outputs live under `data/feature_inputs/vnstock/`.
