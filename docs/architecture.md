...
4. Ingestion campaign and assembly: implemented under `hose_quant.data.campaigns` and the shared workflow boundary. Immutable plans split one observed current-snapshot stock universe into symbol/date tasks. Immutable receipts and child manifests support resume and interruption reconciliation. Fully resolved, compatible campaigns can publish deterministic `assembled-daily-v1` datasets atomically under `data/assembled/vnstock/daily/`, with source run/path lineage on every row. Publication is independent of research-readiness acceptance.
