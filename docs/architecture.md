...
4. Ingestion campaign and assembly: implemented under `hose_quant.data.campaigns`, `hose_quant.data.forensics`, and the shared workflow boundary. Immutable plans split one observed current-snapshot stock universe into symbol/date tasks. Immutable receipts and child manifests support resume and interruption reconciliation. Offline forensic audits classify failed and stale evidence without changing campaign state, weakening validation, or publishing reconstructed data. Fully resolved, compatible campaigns can publish deterministic `assembled-daily-v1` datasets atomically under `data/assembled/vnstock/daily/`, with source run/path lineage on every row. Publication is independent of research-readiness acceptance.
...
Campaign completion, task-source compatibility, assembly publication, coverage-quality acceptance,
research readiness, and canonical candidacy are separate state transitions. A matching
`daily-campaign-readiness-v1` assessment must pass the explicit campaign policy before an assembled
dataset can become a candidate for the bounded raw-OHLCV and VND-liquidity research scope. This is
not proof of historical universe membership, adjusted prices, corporate-action completeness, or
complete exchange-calendar coverage. See
